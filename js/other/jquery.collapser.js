...
(function(jQuery, window, document, undefined) {
	var name = "collapser",
		defaults = {
			...
			showText: 'Expand',
			hideText: 'Hide',
			showClass: 'closed-route',
			hideClass: 'open-route',
			atStart: 'hide',
			lockHide: false,
			dynamic: false,
			changeText: false,
	...
	function Collapser(el, options) {
		var s = this;
		s.o = jQuery.extend({}, defaults, options);
		s.e = jQuery(el);
		s.init();
	}
	Collapser.prototype = {
		init: function() {
			...
			s.mode = s.o.mode;
			s.remaining = {};
			s.ctrlHtml = ' <a href="#" data-ctrl class="' + (!jQuery.isFunction(s.o.controlBtn) ? s.o.controlBtn : '') + '"></a>';
			jQuery(s.e).each(function() {
				// Store the original HTML in a data cache
				jQuery(this).data('oCnt', s.e.html());
				// Get the start type of the target element and activate the collapse
				var atStart = jQuery.isFunction(s.o.atStart) ? s.o.atStart.call(s.e) : s.o.atStart;
		...
		show: function(ele, speed) {
			...
			var callback = function() {
				if (jQuery.isFunction(s.o.afterShow)) s.o.afterShow.call(s.e, s);
			};
			if (jQuery.isFunction(s.o.beforeShow)) s.o.beforeShow.call(s.e, s);
			switch (s.mode) {
				case 'chars':
				case 'words':
				...
				case 'lines':
					...
					if (e.children('div').length == 0) e.wrapInner('<div>');
			...
			if (s.mode == 'block') {
				...
				if (e.find('[data-ctrl]').length == 0 && !jQuery.isFunction(s.o.controlBtn)) {
					e.append(s.ctrlHtml);
				}
				// Bind the on click event of the control button
				s.ctrlBtn = jQuery.isFunction(s.o.controlBtn) ? s.o.controlBtn.call(s.e) : jQuery(e.find('[data-ctrl]'));
		...
		hide: function(ele, speed) {
			...
			// Callbacks
			var callback = function() {
				if (jQuery.isFunction(s.o.afterHide)) s.o.afterHide.call(s.e, s);
			};
			if (jQuery.isFunction(s.o.beforeHide)) s.o.beforeHide.call(s.e, s);
			// Remove the control button already present
			e.find('[data-ctrl]').remove();
			switch (s.mode) {
				case 'chars':
					// Get the text of the element and calculate the remaining chars
					var txt = jQuery.trim(e.text());
		...
		pad: function(a, b) {
			var s = this;
			return a + '<span class="coll-ellipsis">' + s.o.ellipsis + '</span>' + (!jQuery.isFunction(s.o.ctrlBtn) ? s.ctrlHtml : '') + '<span class="coll-hidden" style="display:none">' + b + '</span>';
		},
		...
		blockMode: function(e, type, speed, cb) {
			...
			if (!jQuery.isFunction(s.o.target)) {
				if (jQuery.fn[s.o.target]) jQuery(e)[s.o.target]()[effect](speed, cb);
			}
			else {
			...
			if (type == 'show') {
				e.removeClass(s.o.showClass).addClass(s.o.hideClass);
				if (s.o.changeText) e.text(s.o.hideText);
			}
			else {
				e.removeClass(s.o.hideClass).addClass(s.o.showClass);
				if (s.o.changeText) e.text(s.o.showText);
			}
		},
		reInit: function(e) {
			var s = this;
	...
	jQuery.fn[name] = function(options) {
		return this.each(function() {
			if (!jQuery.data(this, name)) {
				jQuery.data(this, name, new Collapser(this, options));
			}
		});
	};
